...
TP = 0.006
SL = 0.02
TIME_TRADE = 12 # 12 пятиминуток стои в сделке - 60 минут. За это время должны словить тейк или стоп, если не словим - сделка 0
...
for coin_name in coin:
    ...
    for index, row in df.iterrows():
        if index < (len(df)-15): # ограничиваем фрейм сверху, так как не сможем проверить - удачная сделка или нет. Потом нужно будет обрезать рфейм сверху на 16 значений
            # Сначала рассматриваем только лонги
            price_trade = row['open']
            df_trade = df.iloc[index:index+12]
            ...
            for index_now, row_now in df_trade.iterrows(): # условие для лонга
                if row_now['close']>TP_price or row_now['high']>TP_price:
                    ...
                    long_result=-1 # значит сработал стоп
                    break
                else:
            ...
            for index_now, row_now in df_trade.iterrows(): # условие для шорта
                if row_now['close']<TP_price or row_now['low']<TP_price:
                    ...
                    short_result=-1 # значит сработал стоп
                    break
                else:
